...
function initFlowerEngineInTarget(canvasElement) {
    ...
    class Flower {
        ...
        init() {
            ...
            for (let i = 0; i < stemCount; i++) {
                this.stems.push({
                    ...
                    angle: -Math.PI / 2 + (Math.random() - 1.5) * 0.1, // Fixed angle for PMH transition
                    maxLength: (canvas.height - this.y) * (0.9 + Math.random() * 0.2),
                    width: (5 + Math.random() * 2) * scaleFactor,
                    growthSpeed: (2.5 + Math.random() * 2.5) * scaleFactor, // Faster growth for PMH
                    sway: 0,
                    segments: 0
                });
            }
                    ...
                    if (currentDist < stem.maxLength) {
                        ...
                        if (Math.random() < 0.02 && stem.segments > 20) {
                            this.leaves.push({
                                ...
                                size: 0,
                                maxSize: (30 + Math.random() * 30) * scaleFactor,
        ...
        cacheBloom() {
            ...
            sortedPetals.forEach(p => {
                bCtx.save();
                bCtx.rotate(p.angle + p.angleOffset);
        ...
        initBloom() {
            ...
            if (this.type === 'rose') {
                ...
                for (let i = 0; i < count; i++) {
                    const palette = COLOR_MAP[this.type];
                    this.petals.push({
    ...
    window.startFlowerGrowth = (type, color) => {
        ...
        for(let i=0; i<5; i++) {
            setTimeout(() => {
                ...
                flowers.push(new Flower(x, y, currentFlowerType, currentFlowerColor));
            }, i * 1000);
        }
    };
