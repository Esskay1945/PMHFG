...
app.post('/api/generate-link', auth, [
    ...
    body('flowerType').optional().isString(),
    body('flowerColor').optional().isString(),
    body('flowerBgmMode').optional().isString(),
    body('flowerBgmFile').optional().isString()
], validate, async (req, res) => {
    const { name, message, audioFile, flowerType, flowerColor, flowerBgmMode, flowerBgmFile } = req.body;
    ...
    links.set(linkId, {
        ...
        flowerBgmMode: flowerBgmMode || 'same',
        flowerBgmPath: flowerBgmFile ? `/uploads/${flowerBgmFile}` : '',
        status: 'pending',
        createdAt: new Date()
    });
    res.json({ linkId });
...
app.get('/api/get-link', [query('id').isLength({ min: 10, max: 10 })], validate, (req, res) => {
    ...
    res.json({
        ...
        flowerType: linkData.flowerType,
        flowerColor: linkData.flowerColor,
        flowerBgmMode: linkData.flowerBgmMode,
        flowerBgmUrl: linkData.flowerBgmPath,
